...
actions!(
    repl,
    [
        /// Runs the auto detected block
        Run,
        /// Run the line or selection and advances to next line
        RunLine,
        /// Runs the current line or selection without advancing.
        RunInPlace, 
        /// Run the current cell and advances to the next cell.
        RunCell,
        ...
        move |editor: &mut Editor, window, cx: &mut Context<Editor>| {
            ...
            cx.defer_in(window, |editor, _window, cx| {
                ...
                editor
                    .register_action({
                        ...
                        move |_: &Run, window, cx| {
                            ...
                            crate::run(editor_handle.clone(), true, window, cx, ReplRunMode::Block).log_err();
                        }
                    })
                    .detach();
                ...
                editor
                    .register_action({
                        let editor_handle = editor_handle.clone();
                        move |_: &RunCellsAbove, window, cx| {
                            if !JupyterSettings::enabled(cx) {
                                return;
                            }

                            crate::run(editor_handle.clone(), false, window, cx, ReplRunMode::AboveCells).log_err();
                        }
                    })
                    .detach();
